...
## Node details.
FQDN="$(invoke_configcli namespace node fqdn)"
ROLE="$(invoke_configcli namespace node role_id)"
DOMAIN="$(invoke_configcli namespace node domain)"
DISTRO="$(invoke_configcli namespace node distro_id)"
HOSTNAME="$(invoke_configcli namespace node hostname)"
NODEGROUP="$(invoke_configcli namespace node nodegroup_id)"
DEPENDS_ON="$(invoke_configcli namespace node depends_on)"
DTAP_JAR="/opt/bluedata/bluedata-dtap.jar"
CLUSTER_NAME="$(invoke_configcli namespace cluster name)"
TOTAL_VCPU=$(invoke_configcli namespace distros ${DISTRO}.${NODEGROUP}.roles.${ROLE}.flavor.cores)
TOTAL_VMEM=$(invoke_configcli namespace distros ${DISTRO}.${NODEGROUP}.roles.${ROLE}.flavor.memory)
...
CLUSTER_CONFIG_CHOICE() {
    local NGID=$1; shift
    local KEY=$1
    invoke_configcli_ignore_error namespace cluster config_choice_selections.${NGID}.${KEY}
}
...
CLUSTER_CONFIG_METADATA() {
    local NGID=$1; shift
    local KEY=$1
    invoke_configcli_ignore_error namespace cluster config_metadata.${NGID}.${KEY}
}
...
REGISTER_START_SERVICE_COMMON() {
    ...
    SRVC_NODEGRPS=$(invoke_configcli namespace services ${SRVCID} | tr ',' ' ')
    if  grep -wq ${NODEGROUP} <<< ${SRVC_NODEGRPS}; then
        SRVC_ROLES=$(invoke_configcli namespace services ${SRVCID}.${NODEGROUP} | tr ',' ' ')
        if  grep -wq ${ROLE} <<< ${SRVC_ROLES}; then
            ...
            then
                invoke_configcli --service_key "services.${SRVCID}.${NODEGROUP}.${ROLE}" ${REG_OPTION}
            fi
        fi
    fi
}
...
UNIQUE_SELF_NODE_INT() {
    echo $(invoke_configcli macro node --get_self_index)
}
...
UNIQUE_ANOTHER_NODE_INT() {
    echo $(invoke_configcli macro node ---get_index $1)
    return $?
}
...
UNIQUE_INT_ID_BY_SRVC() {
    ...
    if grep -wq ${NODEGROUP} <<< ${SRVC_NODEGRPS}; then
        SRVC_ROLES=$(invoke_configcli namespace services ${SRVCID}.${NODEGROUP} | tr ',' ' ')
        if grep -wq ${ROLE} <<< ${SRVC_ROLES}; then
            HOSTS=($(invoke_configcli namespace services ${SRVCID}.${NODEGROUP}.${ROLE}.fqdns | tr ',' '\n' | sort))
...
## that nodegroups.
##
## This macro is deprecated and should not be used any more.
##
## Input:
##      (nothing)
## Return:
...
GET_FQDN_LIST() {
    role=$1
    ROLE_FQDN="$(invoke_configcli namespace distros ${DISTRO}.${NODEGROUP}.roles.${role}.fqdns)"
    echo $ROLE_FQDN
    return 0
}
...
GET_SERVICE_URL() {
    ...
    if [[ -z ${NODEGRP} ]];
    then
        local NODEGRP_IDS_ARRAY=($(invoke_configcli namespace services ${SRVC} 2>/dev/null | tr ',' '\n' | sort))
    ...
    APP_URL="$(invoke_configcli namespace services ${SRVC}.${NODEGRP}.${SRVC_ROLE}.endpoints)"
    echo "${APP_URL}"
    return 0
}
...
TENANT_INFO() {
    KEY=$1
    TENANT_INFO="$(invoke_configcli_ignore_error namespace tenant 2>/dev/null)"
    if [[ ${TENANT_INFO} == "" ]]; then
        echo ""
    else
        CONFIG_VALUE="$(invoke_configcli_ignore_error namespace tenant.${KEY} 2>/dev/null)"
        echo "${CONFIG_VALUE}"
    fi
    return 0
}
